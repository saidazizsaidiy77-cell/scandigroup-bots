...
//
//  Bo'limlar ekranida ham shu: konver yonida nechtasi buyurtmada ekani
//  turadi, qator bosilsa ostida kim va qachonga kutayotgani chiqadi.
//  Shuning uchun huquq `production.entry` ni ham oladi — tsex ustasida
//  jurnal yo'q, lekin o'z konverini kim kutayotganini bilishi kerak.
//
//  Tsex doirasi bu yerda ham CHEGARA: doirasi bor xodim boshqa tsexning
//  konverini so'rasa ham ochilmaydi.
router.get('/:id/bron', need('production.view', 'production.entry'),
  wrap(async (req, res) => {
  const scope = scopeOf(req);
  if (scope) {
    const ok = (await db.query(
      ...
      [req.params.id, scope])).rowCount;
    if (!ok) return res.status(403).json({ error: 'Bu konver sizning doirangizda emas' });
  }
